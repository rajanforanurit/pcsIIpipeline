...
def _parse_options_from_block(block_text: str) -> Optional[Dict[str, str]]:
    ...
    if len(opts) >= 4:
        return opts
    opts = {}
    matches = _OPT_DEVANAGARI.findall(block_text)
    for key, val in matches:
        k = _DEVANAGARI_OPT_MAP.get(key, '')
...
def _clean_question_text(text: str) -> str:
    ...
    for line in lines:
        ...
        if not stripped:
            continue
        if re.match('^\\(?\\s*[A-Da-dकखगघ]\\s*[).\\-:।]', stripped):
            break
        question_lines.append(stripped)
    return ' '.join(question_lines).strip()
def parse_hindi_questions(text: str, year: int, exam: str, paper: str, set_name: Optional[str]=None, job_id: str='') -> List[Dict[str, Any]]:
    blocks = _extract_question_blocks(text)
    parsed: List[Dict[str, Any]] = []
    for qno, block_text, _, _ in blocks:
        if not _is_primarily_hindi(block_text):
            continue
        ...
        if not question_text or len(question_text) < 5:
            continue
        parsed.append({'job_id': job_id, 'question_no': qno, 'year': year, 'exam': exam, 'paper': paper, 'language': 'Hindi', 'set_name': set_name, 'question': question_text, 'options': {'A': options.get('A', ''), 'B': options.get('B', ''), 'C': options.get('C', ''), 'D': options.get('D', '')}, 'correct_answer': None})
    return parsed
