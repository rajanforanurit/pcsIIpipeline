...
_Q_PATTERNS = [
    ...
    re.compile(r"^प्र\.?\s*(\d{1,3})[.)।]?\s+(.+)", re.MULTILINE),
]
_OPT_PATTERN = re.compile(r"^\s*\(?\s*([A-Da-d])\s*[).\-:।]\s*(.+)", re.MULTILINE)
_OPT_DEVANAGARI = re.compile(r"^\s*[(\[]\s*([कखगघ])\s*[)\]]\s*(.+)", re.MULTILINE)
...
def _clean_question_text(text: str) -> str:
    ...
    for line in lines:
        ...
        if not stripped:
            continue
        if re.match(r"^\(?\s*[A-Da-dकखगघ]\s*[).\-:।]", stripped):
            break
        question_lines.append(stripped)
    return " ".join(question_lines).strip()


def parse_hindi_questions(
    text: str,
    year: int,
    exam: str,
    paper: str,
