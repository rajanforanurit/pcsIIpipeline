...
_Q_PATTERNS = [
    ...
    re.compile(r"^Q\.?\s*(\d{1,3})[.):]?\s+(.+)", re.MULTILINE),
    re.compile(r"^(?:Question|Ques\.?)\s*(\d{1,3})[.):]?\s+(.+)", re.MULTILINE | re.IGNORECASE),
]
_OPT_PATTERN = re.compile(r"^\s*\(?\s*([A-Da-d])\s*[).\-:]\s*(.+)", re.MULTILINE)
_OPT_INLINE = re.compile(r"\b([A-Da-d])\s*[).\-:]\s*(.+?)(?=\s+[A-Da-d]\s*[).\-:]|$)", re.DOTALL)
