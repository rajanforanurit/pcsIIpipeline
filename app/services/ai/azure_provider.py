...
def _build_system_prompt(suggested_count: int | None) -> str:
    if suggested_count is None:
        count_instruction = f'Generate as many questions as the text can support. There is NO upper limit — analyse the full content and create one question per distinct, examinable fact or concept. Minimum {_MIN_QUESTIONS} questions.'
    else:
        count_instruction = f'Generate exactly {suggested_count} questions based on the text provided. If the text cannot support {suggested_count} distinct questions, generate as many as possible (minimum {_MIN_QUESTIONS}).'
    return f'You are an expert UPSC/PSC exam question creator. Generate high-quality multiple choice questions from the provided text.\nRules:\n- Each question must be factual and directly based on the provided text\n- Questions must be UPSC/PSC exam style\n- Each question must have exactly 4 options (A, B, C, D)\n- Only one option must be correct\n- correct_answer must be exactly one of: A, B, C, D\n- explanation must be concise (1-2 sentences)\n- Return ONLY valid JSON array, no markdown, no extra text\n- {count_instruction}\nOutput format (strict JSON array):\n[\n  {{\n    "question": "Question text here?",\n    "options": {{"A": "Option A", "B": "Option B", "C": "Option C", "D": "Option D"}},\n    "correct_answer": "A",\n    "explanation": "Brief explanation"\n  }}\n]'
...
class AzureAIProvider(BaseAIProvider):
    def __init__(self) -> None:
        ...
        self._client = httpx.AsyncClient(timeout=120.0, headers={'Authorization': f'Bearer {self._api_key}', 'Content-Type': 'application/json'})
    async def generate_questions(self, chunk: str, chunk_index: int, book_title: str, subject: str) -> List[Dict[str, Any]]:
        ...
        logger.info('ai.generating_questions', chunk_index=chunk_index, word_count=len(chunk.split()), suggested_count=suggested_count if suggested_count else 'auto', max_tokens=max_tokens)
        count_hint = f'Generate {suggested_count} UPSC/PSC style MCQ questions' if suggested_count else 'Generate as many UPSC/PSC style MCQ questions as the text supports (no upper limit)'
        user_prompt = f'Book: {book_title}\nSubject: {subject}\n\nText:\n{chunk}\n\n{count_hint}. Return only JSON array.'
        payload = {'model': self._model, 'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': user_prompt}], 'max_tokens': max_tokens, 'temperature': 0.7, 'top_p': 0.95}
        try:
            ...
            logger.error('ai.http_error', status=e.response.status_code, chunk_index=chunk_index)
        ...
        try:
            ...
            for q in questions:
                ...
                if q['correct_answer'].upper() not in {'A', 'B', 'C', 'D'}:
                    continue
                validated.append({'question': str(q['question']).strip(), 'options': {'A': str(opts['A']).strip(), 'B': str(opts['B']).strip(), 'C': str(opts['C']).strip(), 'D': str(opts['D']).strip()}, 'correct_answer': q['correct_answer'].upper(), 'explanation': str(q.get('explanation', '')).strip(), 'chunk_index': chunk_index})
    ...
    async def health_check(self) -> bool:
        try:
            url = f'{self._endpoint}?api-version=2024-05-01-preview'
            payload = {'model': self._model, 'messages': [{'role': 'user', 'content': 'ping'}], 'max_tokens': 5}
