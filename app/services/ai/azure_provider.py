...
def _build_system_prompt(suggested_count: int | None) -> str:
    if suggested_count is None:
        count_instruction = (
            ...
            f"There is NO upper limit — analyse the full content and create "
            f"one question per distinct, examinable fact or concept. "
            f"Minimum {_MIN_QUESTIONS} questions."
        )
    else:
        count_instruction = (
    ...
    return (
        f"You are an expert UPSC/PSC exam question creator. Generate high-quality multiple choice questions from the provided text.\n"
        f"Rules:\n"
        f"- Each question must be factual and directly based on the provided text\n"
        f"- Questions must be UPSC/PSC exam style\n"
        f"- Each question must have exactly 4 options (A, B, C, D)\n"
        f"- Only one option must be correct\n"
        f"- correct_answer must be exactly one of: A, B, C, D\n"
        f"- explanation must be concise (1-2 sentences)\n"
...
class AzureAIProvider(BaseAIProvider):
    ...
    async def generate_questions(self, chunk: str, chunk_index: int, book_title: str, subject: str) -> List[Dict[str, Any]]:
        ...
        user_prompt = (
            ...
            f"Subject: {subject}\n\n"
            f"Text:\n{chunk}\n\n"
            f"{count_hint}. Return only JSON array."
        )
        payload = {
            "model": self._model,
            "messages": [
