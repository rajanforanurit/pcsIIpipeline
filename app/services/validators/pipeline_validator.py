...
def normalise_question(q: Dict[str, Any]) -> Dict[str, Any]:
    ca = q.get('correct_answer')
    return {'id': int(q['id']), 'year': int(q['year']) if q.get('year') else None, 'exam': str(q.get('exam', '')).strip(), 'paper': str(q.get('paper', '')).strip(), 'language': str(q.get('language', 'English')).strip(), 'question': str(q['question']).strip(), 'options': {'A': str(q['options']['A']).strip(), 'B': str(q['options']['B']).strip(), 'C': str(q['options']['C']).strip(), 'D': str(q['options']['D']).strip()}, 'correct_answer': str(ca).upper() if ca else None}
