...
def generate_validation_report(valid: List[Dict[str, Any]], invalid: List[Dict[str, Any]], total_parsed: int) -> Dict[str, Any]:
    return {'total_parsed': total_parsed, 'valid_count': len(valid), 'invalid_count': len(invalid), 'pass_rate_percent': round(len(valid) / total_parsed * 100, 2) if total_parsed > 0 else 0, 'issues': invalid[:50]}
def validate_bilingual_questions(raw_questions: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    valid: List[Dict[str, Any]] = []
    invalid: List[Dict[str, Any]] = []
    seen_qnos: set = set()
    for idx, q in enumerate(raw_questions):
        errors = _validate_bilingual_single(q, seen_qnos)
        if errors:
            invalid.append({'index': idx, 'question_no': q.get('question_no'), 'errors': errors, 'data': q})
        else:
            seen_qnos.add(q['question_no'])
            valid.append(q)
    return (valid, invalid)
def _validate_bilingual_single(q: Dict[str, Any], seen_qnos: set) -> List[str]:
    errors: List[str] = []
    qno = q.get('question_no')
    if not isinstance(qno, int) or qno < 1:
        errors.append(f'Invalid question_no: {qno}')
    ...
    if not english and (not hindi):
        errors.append('Neither English nor Hindi content was parsed for this question')
        return errors
    for label, content in (('english', english), ('hindi', hindi)):
        if content is None:
            continue
        question_text = content.get('question', '')
        if not question_text or len(str(question_text).strip()) < _MIN_QUESTION_LEN:
            errors.append(f'{label} question text is missing or too short')
        options = content.get('options', {})
        if not isinstance(options, dict):
            errors.append(f'{label} options must be a dictionary')
            continue
        missing_opts = _OPTION_KEYS - set(options.keys())
        if missing_opts:
            errors.append(f'{label} missing options: {missing_opts}')
        else:
            for key in _OPTION_KEYS:
                val = options.get(key, '')
                if not val or len(str(val).strip()) < _MIN_OPTION_LEN:
                    errors.append(f'{label} option {key} is empty or too short')
    return errors
def generate_bilingual_validation_report(valid: List[Dict[str, Any]], invalid: List[Dict[str, Any]], total_parsed: int) -> Dict[str, Any]:
    return {'total_parsed': total_parsed, 'valid_count': len(valid), 'invalid_count': len(invalid), 'pass_rate_percent': round(len(valid) / total_parsed * 100, 2) if total_parsed > 0 else 0, 'issues': invalid[:50]}
