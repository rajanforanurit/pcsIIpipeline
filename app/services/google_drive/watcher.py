...
async def poll_new_pdfs(page_token: str) -> tuple[List[Dict], str]:
    ...
    def _poll() -> tuple[List[Dict], str]:
        ...
        token = page_token
        while True:
            resp = service.changes().list(pageToken=token, spaces='drive', fields='nextPageToken,newStartPageToken,changes(fileId,removed,file(id,name,mimeType,md5Checksum,size,parents,trashed))', includeRemoved=False).execute()
...
async def list_existing_pdfs_in_unprocessed() -> List[Dict]:
    ...
    def _list() -> List[Dict]:
        ...
        while True:
            resp = service.files().list(q=query, spaces='drive', fields='nextPageToken,files(id,name,mimeType,md5Checksum,size,parents)', pageToken=page_token).execute()
