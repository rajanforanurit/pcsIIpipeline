"""Upload files (JSON, logs) to Google Drive folders."""
import asyncio
import io
...
async def upload_json(folder_id: str, file_name: str, data: Any) -> str:
    """Upload a Python object as JSON to a Drive folder. Returns the new file ID."""
...
async def upload_text(folder_id: str, file_name: str, content: str, mime_type: str = "text/plain") -> str:
    """Upload plain text content to Drive. Returns new file ID."""
    loop = asyncio.get_event_loop()
    service = get_drive_service()
