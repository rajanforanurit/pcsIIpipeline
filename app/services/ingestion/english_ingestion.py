...
from app.repositories.question_repository import EnglishQuestionRepository  # changed
...
async def run_english_ingestion(job_id: str, db: AsyncIOMotorDatabase) -> None:
    job_repo = JobRepository(db)
    q_repo = EnglishQuestionRepository(db)  # changed — writes to englishquestions
    try:
        ...
        await job_repo.append_log(job_id, JobLog(level="INFO", message="English ingestion started"))
        ...
        await job_repo.append_log(job_id, JobLog(level="INFO", message="Parsing complete", details={"raw_count": len(raw)}))
        ...
        await job_repo.mark_finished(
            ...
            total_questions_found=len(raw),
            total_questions_saved=len(inserted_ids),
        )
        await job_repo.append_log(job_id, JobLog(level="INFO", message="Ingestion complete", details={"saved": len(inserted_ids)}))
        logger.info("ingestion.english.complete", job_id=job_id, saved=len(inserted_ids))

    except Exception as exc:
        tb = traceback.format_exc()
        logger.error("ingestion.english.failed", job_id=job_id, error=str(exc))
        await job_repo.append_error(job_id, JobError(stage="english_ingestion", message=str(exc), traceback=tb))
