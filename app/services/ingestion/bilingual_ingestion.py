...
from app.models.common import JobStatus, PDFType, QuestionStatus
...
async def run_bilingual_ingestion(job_id: str, db: AsyncIOMotorDatabase) -> None:
    ...
    try:
        ...
        for block in question_blocks:
            ...
            eng_parsed = parse_english_questions(
                text=eng_synthetic, year=year, exam=exam, paper=paper, set_name=set_name, job_id=job_id
            ) if eng_synthetic else []

            hin_parsed = parse_hindi_questions(
                text=hin_synthetic, year=year, exam=exam, paper=paper, set_name=set_name, job_id=job_id
            ) if hin_synthetic else []
            ...
            await job_repo.append_log(job_id, JobLog(
                level='INFO',
                message=f'Question {qno} split into English/Hindi',
                details={
                    'hindi_ratio': ratios['hindi_ratio'],
                    'english_ratio': ratios['english_ratio'],
                    'english_found': english_data is not None,
                    'hindi_found': hindi_data is not None,
                },
            ))
        ...
        questions: List[PCSQuestion] = []

        for q in valid:
            english = LanguageContent(
                question=q['english']['question'],
                options=QuestionOptions(**q['english']['options']),
            ) if q.get('english') else None

            hindi = LanguageContent(
                question=q['hindi']['question'],
                options=QuestionOptions(**q['hindi']['options']),
            ) if q.get('hindi') else None

            questions.append(PCSQuestion(
                job_id=job_id,
        ...
        for q, qid in zip(valid, inserted_ids):
            await job_repo.append_log(job_id, JobLog(
                level='INFO',
                message=f"Stored question {exam_token}{year}_Q{q['question_no']}",
