...
async def run_book_ingestion(job_id: str, db: AsyncIOMotorDatabase) -> None:
    ...
    try:
        ...
        questions = [BookQuestion(job_id=job_id, book_title=book_title, book_author=book_author if book_author else None, subject=subject if subject else None, chunk_index=q['chunk_index'], question=q['question'], options=QuestionOptions(**q['options']), correct_answer=q['correct_answer'], explanation=q.get('explanation'), created_by=created_by, updated_by=created_by) for q in valid]
