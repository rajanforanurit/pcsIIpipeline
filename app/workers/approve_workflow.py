...
async def run_approve_workflow(review_id: str, questions: List[Dict[str, Any]], db: AsyncIOMotorDatabase, approved_by: str='admin') -> Dict[str, Any]:
    ...
    q_repo = HindiQuestionRepository(db) if language == Language.HINDI else EnglishQuestionRepository(db)
    pcs_questions = []
    for q in normalised:
        pcs_q = PCSQuestion(job_id=drive_job_id, question_no=q['id'], year=q['year'] or 0, exam=q['exam'] or '', paper=q['paper'] or '', language=language, question=q['question'], options=QuestionOptions(**q['options']), correct_answer=q.get('correct_answer'), status=QuestionStatus.APPROVED, created_by=approved_by, updated_by=approved_by)
    ...
    if pending_json_id and settings.GOOGLE_DRIVE_PROCESSED_JSON_FOLDER_ID:
        ok = await move_file(pending_json_id, settings.GOOGLE_DRIVE_PROCESSED_JSON_FOLDER_ID, settings.GOOGLE_DRIVE_PENDING_JSON_FOLDER_ID)
        if not ok:
    ...
    if drive_file_id and settings.GOOGLE_DRIVE_PROCESSED_PDF_FOLDER_ID:
        ok = await move_file(drive_file_id, settings.GOOGLE_DRIVE_PROCESSED_PDF_FOLDER_ID, settings.GOOGLE_DRIVE_UNPROCESSED_FOLDER_ID)
        if not ok:
            move_errors.append('Failed to move PDF to processed folder')
    finished_at = datetime.now(timezone.utc)
    try:
        await upload_processing_log(file_name=file_name, drive_file_id=drive_file_id, started_at=started_at, finished_at=finished_at, total_pages=review_doc.get('total_pages', 0), ocr_duration_seconds=0.0, ai_duration_seconds=0.0, question_count=len(inserted_ids), document_type=document_type, language=language_str, errors=move_errors, warnings=[f'Invalid questions skipped: {len(invalid)}'] if invalid else [], outcome='approved')
    except Exception:
        pass
    ...
    return {'success': True, 'inserted_count': len(inserted_ids), 'invalid_count': len(invalid), 'move_errors': move_errors}
async def run_reject_workflow(review_id: str, reason: Optional[str], db: AsyncIOMotorDatabase) -> Dict[str, Any]:
    repo = DriveRepository(db)
    review_doc = await repo.get_pending_review(review_id)
    if not review_doc:
        return {'success': False, 'errors': [f'Review {review_id} not found']}
    ...
    if pending_json_id and settings.GOOGLE_DRIVE_FAILED_FOLDER_ID:
        await move_file(pending_json_id, settings.GOOGLE_DRIVE_FAILED_FOLDER_ID, settings.GOOGLE_DRIVE_PENDING_JSON_FOLDER_ID)
    ...
    try:
        ...
        await upload_processing_log(file_name=file_name, drive_file_id=drive_file_id, started_at=now, finished_at=now, total_pages=0, ocr_duration_seconds=0.0, ai_duration_seconds=0.0, question_count=0, document_type=review_doc.get('document_type', 'Unknown'), language=review_doc.get('language', 'Unknown'), errors=[f'Rejected by admin. Reason: {reason}' if reason else 'Rejected by admin.'], warnings=[], outcome='rejected')
    except Exception:
        pass
