"""
Google Drive ETL Pipeline Worker.

Stages:
  1. Download PDF from Drive
  2. OCR / Text extraction
  3. DeepSeek AI extraction
  4. Validation
  5. Upload pending JSON to Drive
  6. Create PendingReviewDocument in MongoDB
"""
import asyncio
import os
import time
...
async def run_drive_pipeline(
    drive_file_id: str,
    file_name: str,
    db: AsyncIOMotorDatabase,
) -> bool:
    """
    Full ETL pipeline for a single Drive PDF.
    Returns True on success (pending JSON uploaded), False on failure.
    """
    ...
    try:
        ...
        # ------------------------------------------------------------------ #
        # Stage 3: AI extraction (chunk if large)
        # ------------------------------------------------------------------ #
        ...
        warnings: List[str] = [
            f"Question {q.get('id', '?')} invalid: {q.get('_validation_errors', [])}"
            for q in invalid
        ]
        ...
        await repo.update_drive_job_finished(
            job_id=job_id,
            ...
            processing_time=total_s,
            warnings=warnings,
        )
        await repo.mark_drive_file_processed(
            drive_file_id=drive_file_id,
            job_id=job_id,
        ...
        logger.info(
            "drive_pipeline.complete",
            job_id=job_id,
            questions=len(normalised),
            pending_json=pending_json_id,
        ...
        try:
            await move_file(drive_file_id, settings.GOOGLE_DRIVE_FAILED_FOLDER_ID,
