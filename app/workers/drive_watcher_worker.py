"""
Background asyncio task that continuously watches Google Drive for new PDFs
and feeds them into the ETL pipeline.
"""
import asyncio
...
from app.services.google_drive.watcher import (
    get_start_page_token,
    list_existing_pdfs_in_unprocessed,
    poll_new_pdfs,
)
...
async def _process_file(drive_file_id: str, file_name: str) -> None:
    """Process a single PDF through the pipeline (respects concurrency limit)."""
    sem = _get_semaphore()
    async with sem:
        db = MongoDB.get_db()
...
async def _initial_scan() -> None:
    """On startup, pick up any PDFs already in the unprocessed folder."""
    try:
        ...
        tasks = [
            asyncio.create_task(_process_file(f["id"], f["name"]))
            for f in files
        ]
...
async def watch_loop() -> None:
    """Continuous polling loop using the Drive Changes API."""
    logger.info("drive_watcher.starting", interval=settings.CHECK_INTERVAL_SECONDS)

    db = MongoDB.get_db()
    repo = DriveRepository(db)

    # Restore or create page token
