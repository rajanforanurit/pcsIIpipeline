...
async def list_hindi_questions(job_id: Optional[str]=Query(default=None), status_filter: Optional[str]=Query(default=None, alias='status'), exam: Optional[str]=Query(default=None), year: Optional[int]=Query(default=None), search: Optional[str]=Query(default=None), page: int=Query(default=1, ge=1), page_size: int=Query(default=20, ge=1, le=100), current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> PaginatedResponse:
...
async def get_hindi_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)):
    repo = HindiQuestionRepository(db)
...
async def update_hindi_question(question_id: str, payload: UpdatePCSQuestionRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = HindiQuestionRepository(db)
    doc = await repo.get_by_id(question_id)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Question {question_id} not found')
...
async def delete_hindi_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = HindiQuestionRepository(db)
...
async def approve_hindi_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = HindiQuestionRepository(db)
...
async def bulk_approve_hindi_questions(payload: BulkApproveRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = HindiQuestionRepository(db)
