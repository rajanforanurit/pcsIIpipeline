...
async def list_book_questions(job_id: Optional[str]=Query(default=None), book_title: Optional[str]=Query(default=None), status_filter: Optional[str]=Query(default=None, alias='status'), search: Optional[str]=Query(default=None), page: int=Query(default=1, ge=1), page_size: int=Query(default=20, ge=1, le=100), current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> PaginatedResponse:
...
async def export_approved_book_questions(job_id: Optional[str]=Query(default=None), current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)):
    repo = BookQuestionRepository(db)
...
async def get_book_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)):
    repo = BookQuestionRepository(db)
...
async def update_book_question(question_id: str, payload: UpdateBookQuestionRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = BookQuestionRepository(db)
    doc = await repo.get_by_id(question_id)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Question {question_id} not found')
...
async def delete_book_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = BookQuestionRepository(db)
...
async def approve_book_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = BookQuestionRepository(db)
...
async def bulk_approve_book_questions(payload: BulkApproveRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = BookQuestionRepository(db)
