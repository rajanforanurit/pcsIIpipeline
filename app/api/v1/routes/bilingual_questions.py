...
router = APIRouter(prefix='/questions/pcs', tags=['PCS Questions'])
...
async def delete_pcs_question(
    ...
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Question {question_id} not found')
    logger.info('question.pcs.deleted', question_id=question_id, admin=current_admin)
...
async def approve_pcs_question(
    question_id: str,
    current_admin: str = Depends(get_current_admin),
    db: AsyncIOMotorDatabase = Depends(get_db),
) -> SuccessResponse:
...
async def bulk_approve_pcs_questions(
    ...
    logger.info('question.pcs.bulk_approved', count=count, admin=current_admin)
