...
async def list_pending_reviews(review_status: str=Query(default='pending', alias='status'), page: int=Query(default=1, ge=1), page_size: int=Query(default=20, ge=1, le=100), current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> PaginatedResponse:
    ...
    items = []
    for doc in docs:
        item = {'id': str(doc.get('_id', '')), 'file_name': doc.get('file_name', ''), 'document_type': doc.get('document_type'), 'language': doc.get('language'), 'year': doc.get('year'), 'exam': doc.get('exam'), 'paper': doc.get('paper'), 'question_count': doc.get('question_count', 0), 'status': doc.get('status', 'pending'), 'created_at': doc.get('created_at'), 'drive_job_id': doc.get('drive_job_id')}
        items.append(item)
...
async def get_pending_review(review_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> Dict[str, Any]:
    repo = DriveRepository(db)
...
async def update_review_questions(review_id: str, payload: ApproveReviewRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = DriveRepository(db)
    doc = await repo.get_pending_review(review_id)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Review {review_id} not found')
...
async def approve_review(review_id: str, payload: ApproveReviewRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    ...
    msg = f'Approved. Inserted {result['inserted_count']} questions into MongoDB. Skipped {result.get('invalid_count', 0)} invalid questions.'
...
async def reject_review(review_id: str, payload: RejectReviewRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    doc = await DriveRepository(db).get_pending_review(review_id)
    if not doc:
        raise HTTPException(status_code=404, detail=f'Review {review_id} not found')
    if doc.get('status') != 'pending':
...
async def preview_review_json(review_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> Dict[str, Any]:
    repo = DriveRepository(db)
    doc = await repo.get_pending_review(review_id)
    if not doc:
        raise HTTPException(status_code=404, detail=f'Review {review_id} not found')
    return {'file_name': doc.get('file_name'), 'document_type': doc.get('document_type'), 'language': doc.get('language'), 'year': doc.get('year'), 'exam': doc.get('exam'), 'paper': doc.get('paper'), 'question_count': doc.get('question_count', 0), 'questions': doc.get('questions', [])}
