...
from app.repositories.question_repository import EnglishQuestionRepository
router = APIRouter(prefix='/questions/english', tags=['English Questions Review'])
...
async def list_english_questions(job_id: Optional[str]=Query(default=None), status_filter: Optional[str]=Query(default=None, alias='status'), exam: Optional[str]=Query(default=None), year: Optional[int]=Query(default=None), search: Optional[str]=Query(default=None), page: int=Query(default=1, ge=1), page_size: int=Query(default=20, ge=1, le=100), current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> PaginatedResponse:
    repo = EnglishQuestionRepository(db)
...
async def get_english_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)):
    repo = EnglishQuestionRepository(db)
...
async def update_english_question(question_id: str, payload: UpdatePCSQuestionRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = EnglishQuestionRepository(db)
    doc = await repo.get_by_id(question_id)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Question {question_id} not found')
    ...
    logger.info('question.english.updated', question_id=question_id, admin=current_admin)
    return SuccessResponse(message='Question updated successfully')
@router.delete('/{question_id}', response_model=SuccessResponse)
async def delete_english_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = EnglishQuestionRepository(db)
    deleted = await repo.delete_question(question_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Question {question_id} not found')
    logger.info('question.english.deleted', question_id=question_id, admin=current_admin)
...
async def approve_english_question(question_id: str, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = EnglishQuestionRepository(db)
    ...
    logger.info('question.english.approved', question_id=question_id, admin=current_admin)
    return SuccessResponse(message='Question approved successfully')
@router.post('/bulk/approve', response_model=SuccessResponse)
async def bulk_approve_english_questions(payload: BulkApproveRequest, current_admin: str=Depends(get_current_admin), db: AsyncIOMotorDatabase=Depends(get_db)) -> SuccessResponse:
    repo = EnglishQuestionRepository(db)
    count = await repo.bulk_approve(payload.question_ids, updated_by=current_admin)
    logger.info('question.english.bulk_approved', count=count, admin=current_admin)
