...
from app.repositories.question_repository import (
    ...
    EnglishQuestionRepository,
    HindiQuestionRepository,
)
...
async def get_dashboard_stats(
    ...
    # Combine english + hindi counts for dashboard totals
    total_pcs = sum(en_status_counts.values()) + sum(hi_status_counts.values())
    ...
    return DashboardStats(
        ...
        draft_ready_jobs=job_status_counts.get("Draft Ready", 0),
        total_pcs_questions=total_pcs,
        total_book_questions=total_book,
        draft_pcs_questions=en_status_counts.get("draft", 0) + hi_status_counts.get("draft", 0),
        draft_book_questions=book_status_counts.get("draft", 0),
        approved_pcs_questions=en_status_counts.get("approved", 0) + hi_status_counts.get("approved", 0),
        approved_book_questions=book_status_counts.get("approved", 0),
    )