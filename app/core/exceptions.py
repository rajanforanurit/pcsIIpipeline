...
class JobStateError(PlatformException):
    def __init__(self, job_id: str, current_state: str, required_state: str) -> None:
        super().__init__(f"Job {job_id} is in state '{current_state}', expected '{required_state}'", status_code=409, details={'job_id': job_id, 'current_state': current_state, 'required_state': required_state})
...
class GoogleDriveNotConfiguredError(PlatformException):
    def __init__(self) -> None:
        super().__init__('Google Drive is not configured. Set GOOGLE_SERVICE_ACCOUNT_JSON and folder IDs.', status_code=503)
