...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    # Start Drive watcher (no-op if not configured or PIPELINE_MODE=manual)
    await start_watcher()
...
app = FastAPI(
    ...
    description=(
        "Production backend for UPSC/PSC question ingestion via Google Drive ETL pipeline, "
        "OCR, AI generation (DeepSeek V3.2), admin review, and MongoDB storage."
    ),
    version="2.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def readiness_check():
    ...
    try:
        ...
        return JSONResponse(
            status_code=503,
            content={"status": "not ready", "mongodb": str(exc)},
        )
