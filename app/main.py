...
app = FastAPI(title='AI Question Ingestion Platform', description='Production backend for UPSC/PSC question ingestion via Google Drive ETL pipeline, OCR, AI generation (DeepSeek V3.2), admin review, and MongoDB storage.', version='2.0.0', docs_url='/docs', redoc_url='/redoc', openapi_url='/openapi.json', lifespan=lifespan)
...
async def platform_exception_handler(request: Request, exc: PlatformException) -> JSONResponse:
    ...
    return JSONResponse(status_code=exc.status_code, content={'success': False, 'message': exc.message, 'details': exc.details, 'status_code': exc.status_code})
@app.exception_handler(Exception)
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    logger.error('unhandled.exception', error=str(exc), path=request.url.path, exc_info=True)
    return JSONResponse(status_code=500, content={'success': False, 'message': 'Internal server error', 'details': {}, 'status_code': 500})
...
@app.get('/', tags=['Root'])
async def root():
    return {'message': 'PCS-II Question Ingestion Platform', 'version': '2.0.0', 'docs': '/docs', 'features': ['Google Drive ETL Pipeline', 'Admin Pending Review Panel', 'DeepSeek V3.2 AI Extraction', 'OCR (PaddleOCR)', 'MongoDB Storage']}
application = app
