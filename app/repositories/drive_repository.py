...
class DriveRepository:
    ...
    async def save_watcher_state(self, page_token: str) -> None:
        await self._watcher.update_one({'_id': 'singleton'}, {'$set': {'page_token': page_token, 'updated_at': datetime.now(timezone.utc)}}, upsert=True)
    ...
    async def mark_drive_file_processed(self, drive_file_id: str, job_id: Optional[str], status: str, error_message: Optional[str]=None) -> None:
        update: Dict[str, Any] = {'processed': status not in ('new', 'processing'), 'processing_status': status, 'drive_job_id': job_id, 'updated_at': datetime.now(timezone.utc)}
        if error_message:
            update['error_message'] = error_message
        await self._files.update_one({'drive_file_id': drive_file_id}, {'$set': update}, upsert=True)
    ...
    async def update_job_status(self, job_id: str, status: JobStatus) -> None:
        await self._jobs.update_one({'_id': ObjectId(job_id)}, {'$set': {'status': status.value, 'updated_at': datetime.now(timezone.utc)}})
    async def update_job_field(self, job_id: str, field: str, value: Any) -> None:
        await self._jobs.update_one({'_id': ObjectId(job_id)}, {'$set': {field: value, 'updated_at': datetime.now(timezone.utc)}})
    async def update_drive_job_finished(self, job_id: str, status: JobStatus, pending_json_drive_id: Optional[str], question_count: int, ocr_duration: float, ai_duration: float, processing_time: float, warnings: List[str], errors: Optional[List[str]]=None) -> None:
        update: Dict[str, Any] = {'status': status.value, 'question_count': question_count, 'ocr_duration_seconds': round(ocr_duration, 2), 'ai_duration_seconds': round(ai_duration, 2), 'processing_time_seconds': round(processing_time, 2), 'warnings': warnings, 'finished_at': datetime.now(timezone.utc), 'updated_at': datetime.now(timezone.utc)}
    ...
    async def list_drive_jobs(self, status: Optional[str]=None, skip: int=0, limit: int=20) -> List[Dict[str, Any]]:
        query: Dict[str, Any] = {}
        if status:
            query['status'] = status
    ...
    async def list_pending_reviews(self, status: str='pending', skip: int=0, limit: int=20) -> List[Dict[str, Any]]:
        cursor = self._reviews.find({'status': status}).sort('created_at', -1).skip(skip).limit(limit)
        return await cursor.to_list(length=limit)
    async def count_pending_reviews(self, status: str='pending') -> int:
        return await self._reviews.count_documents({'status': status})
    async def update_pending_review_questions(self, review_id: str, questions: List[Dict[str, Any]]) -> bool:
        result = await self._reviews.update_one({'_id': ObjectId(review_id)}, {'$set': {'questions': questions, 'question_count': len(questions), 'updated_at': datetime.now(timezone.utc)}})
        return result.modified_count > 0
    async def set_pending_review_status(self, review_id: str, status: str) -> bool:
        result = await self._reviews.update_one({'_id': ObjectId(review_id)}, {'$set': {'status': status, 'updated_at': datetime.now(timezone.utc)}})
        return result.modified_count > 0
