...
class JobRepository:
    ...
    async def mark_started(self, job_id: str) -> None:
        await self._col.update_one(
            {"_id": ObjectId(job_id)},
            {"$set": {"status": JobStatus.RUNNING.value, "started_at": datetime.now(timezone.utc), "updated_at": datetime.now(timezone.utc)}},
        )
    ...
    async def update_pdf_info(self, job_id: str, pdf_type: str, total_pages: int) -> None:
        await self._col.update_one(
            {"_id": ObjectId(job_id)},
            {"$set": {"pdf_type": pdf_type, "total_pages": total_pages, "updated_at": datetime.now(timezone.utc)}},
        )

    async def append_log(self, job_id: str, log_entry: JobLog) -> None:
        await self._col.update_one(
            {"_id": ObjectId(job_id)},
            {"$push": {"logs": log_entry.model_dump()}, "$set": {"updated_at": datetime.now(timezone.utc)}},
        )

    async def append_error(self, job_id: str, error: JobError) -> None:
        await self._col.update_one(
            {"_id": ObjectId(job_id)},
            {"$push": {"errors": error.model_dump()}, "$set": {"updated_at": datetime.now(timezone.utc)}},
        )
