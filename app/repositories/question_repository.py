...
from app.db.mongodb import COLLECTION_PCS_QUESTIONS, COLLECTION_BOOK_QUESTIONS
from app.models.common import QuestionStatus
from app.models.question import PCSQuestion, BookQuestion

logger = get_logger(__name__)


class PCSQuestionRepository:
    def __init__(self, db: AsyncIOMotorDatabase) -> None:
        self._col = db[COLLECTION_PCS_QUESTIONS]
    ...
    async def insert_many(self, questions: List[PCSQuestion]) -> List[str]:
        ...
        logger.info('pcsquestions.inserted', count=len(ids))
        return ids
    ...
    async def bulk_approve(self, question_ids: List[str], updated_by: str = 'admin') -> int:
        ...
        logger.info('pcsquestions.bulk_approved', count=result.modified_count)
...
class BookQuestionRepository:
    ...
    async def insert_many(self, questions: List[BookQuestion]) -> List[str]:
        ...
        result = await self._col.insert_many(docs)
        ids = [str(oid) for oid in result.inserted_ids]
        logger.info('bookquestions.inserted', count=len(ids))
        return ids
    ...
    async def bulk_approve(self, question_ids: List[str], updated_by: str = 'admin') -> int:
        ...
        logger.info('bookquestions.bulk_approved', count=result.modified_count)
        return result.modified_count
