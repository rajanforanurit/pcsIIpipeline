...
from app.db.mongodb import COLLECTION_BILINGUAL_QUESTIONS, COLLECTION_BOOK_QUESTIONS, COLLECTION_ENGLISH_QUESTIONS, COLLECTION_HINDI_QUESTIONS
from app.models.common import QuestionStatus
from app.models.question import BilingualQuestion, BookQuestion, PCSQuestion
...
class _BaseQuestionRepository:
    ...
    async def list_questions(self, job_id: Optional[str]=None, status: Optional[str]=None, exam: Optional[str]=None, year: Optional[int]=None, search: Optional[str]=None, skip: int=0, limit: int=20) -> List[Dict[str, Any]]:
    ...
    async def count_questions(self, job_id: Optional[str]=None, status: Optional[str]=None, exam: Optional[str]=None, year: Optional[int]=None, search: Optional[str]=None) -> int:
        query = self._build_query(job_id, status, exam, year, search)
        return await self._col.count_documents(query)
    def _build_query(self, job_id: Optional[str], status: Optional[str], exam: Optional[str], year: Optional[int], search: Optional[str]) -> Dict[str, Any]:
        query: Dict[str, Any] = {}
        if job_id:
            query['job_id'] = job_id
        if status:
        ...
        return query
    async def update_question(self, question_id: str, update_data: Dict[str, Any], updated_by: str='admin') -> bool:
        update_data['updated_at'] = datetime.now(timezone.utc)
        update_data['updated_by'] = updated_by
        result = await self._col.update_one({'_id': ObjectId(question_id)}, {'$set': update_data, '$inc': {'revision_number': 1}})
        return result.modified_count > 0
    async def approve_question(self, question_id: str, updated_by: str='admin') -> bool:
        return await self.update_question(question_id, {'status': QuestionStatus.APPROVED.value}, updated_by)
    async def bulk_approve(self, question_ids: List[str], updated_by: str='admin') -> int:
        ...
        if not object_ids:
            return 0
        result = await self._col.update_many({'_id': {'$in': object_ids}}, {'$set': {'status': QuestionStatus.APPROVED.value, 'updated_at': datetime.now(timezone.utc), 'updated_by': updated_by}, '$inc': {'revision_number': 1}})
...
class PCSQuestionRepository(EnglishQuestionRepository):
    pass
class BilingualQuestionRepository(_BaseQuestionRepository):
    _collection_name = COLLECTION_BILINGUAL_QUESTIONS
class BookQuestionRepository:
    ...
    async def get_by_id(self, question_id: str) -> Optional[Dict[str, Any]]:
        if not ObjectId.is_valid(question_id):
            return None
        return await self._col.find_one({'_id': ObjectId(question_id)})
    async def list_questions(self, job_id: Optional[str]=None, book_title: Optional[str]=None, status: Optional[str]=None, search: Optional[str]=None, skip: int=0, limit: int=20) -> List[Dict[str, Any]]:
        ...
        return await cursor.to_list(length=limit)
    async def count_questions(self, job_id: Optional[str]=None, book_title: Optional[str]=None, status: Optional[str]=None, search: Optional[str]=None) -> int:
        query = self._build_query(job_id, book_title, status, search)
        return await self._col.count_documents(query)
    def _build_query(self, job_id: Optional[str], book_title: Optional[str], status: Optional[str], search: Optional[str]) -> Dict[str, Any]:
        query: Dict[str, Any] = {}
        if job_id:
            query['job_id'] = job_id
        if book_title:
